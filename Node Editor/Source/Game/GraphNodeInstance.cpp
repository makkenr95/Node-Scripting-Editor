...
{
	assert(aGraphManager && "GraphNodeInstance requires access to its GraphManager!");
}
...
void GraphNodeInstance::ExitVia(unsigned int aPinIndex)
{
	...
	std::vector< GraphNodeEdge*> links = GetLinkFromPin(pin.UID.ToUInt());
	for (auto link : links)
	{
		link->Target->Enter();
	}
...
{
	return GetLinkFromPin(aPin.UID.ToUInt()).size() > 0;
}
...
std::vector< GraphNodeEdge*> GraphNodeInstance::GetLinkFromPin(unsigned int aPinToFetchFrom)
{
...
void GraphNodeInstance::FetchDataRaw(DataType& outType, DataPtr& someData, size_t& outSize, unsigned int aPinToFetchFrom)
{
	...
	if (!myNodeType->IsFlowNode())
	{
		...
		{
			std::vector< GraphNodeEdge*> links = GetLinkFromPin(myPins[aPinToFetchFrom].UID.ToUInt());
			if (links.size() > 0)
			{
				...
				GraphManager::ShowFlow(links[0]->LinkID);
